...
export class DownloadDocComponent implements OnInit {
  ...
  constructor(
    private DocsService: DocRequestService,
  ...
  downloadFile(event, filepath: String, fileOriginalname: String, filename: String, hash: String) {
    this.loadingbtn = true;
    console.log('---in download----------');

    var body = { filepath: filepath };
    this._fileService
      .downloadFile(filepath, fileOriginalname, filename, hash)
      .subscribe(result => saveAs(result, filename), error => console.error(error));
  }
